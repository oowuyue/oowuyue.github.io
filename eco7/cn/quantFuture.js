...
async function getSinaFuture() {

    var browser
    var initBrowser = async () => {
        if (await wait(10) && browser) return
    ...
    var getDataFromUrlFunc = async (dataName, dataCode) => {
        let pageUrl = `https://finance.sina.com.cn/futures/quotes/${dataCode}.shtml`
        let apiSub = "InnerFuturesNewService.getDailyKLine?symbol="
        if (dataName.includes("us")) {
            apiSub = "GlobalFuturesService.getGlobalFuturesDailyKLine?symbol="
        }
...
function backTest1(dataName, dayDatas, currentDayIndex, currentDayList, currentWeekList, currentMonthList, trigger1LogArr) {
    ...
    function trigger1Log(currentDayList, currentWeekList, currentMonthList) {
        let currentDayIndex = currentDayList.length - 1
        let currentDayData = currentDayList[currentDayIndex]

        let logProfileN = {}
        logProfileN.name = dataName
        logProfileN.trig1Date = currentDayData.date
        logProfileN.trig1AtIndex = currentDayIndex
        logProfileN.trig1Fucked = false
        ...
        console.log(dataName, "trig1Date", logProfileN.trig1Date)
        trigger1LogArr.push(logProfileN)
    }
    ...
    let currentDay = currentDayList[currentDayList.length - 1]
    let conditon1 = dayCross && dayNlowM && weekJup && monthJup && monthJlow5 && monthPre5Jlow0 && volumeUp
    if (true
        && conditon1 === true // 满足conditon1
        && (trigger1LogArr.length == 0 || currentDayIndex - (trigger1LogArr[trigger1LogArr.length - 1].trig1AtIndex) > 10) //隔10天以上
    ) {
        trigger1Log(currentDayList, currentWeekList, currentMonthList)
        return [true, trigger1LogArr]
    }
    return [false, trigger1LogArr]
}

function backTest12(dataName, dayDatas, currentDayIndex, currentDayList, trigger1LogArr, trigger12LogArr) {

    if (trigger1LogArr.length == 0 || trigger1LogArr[trigger1LogArr.length - 1].trig1Fucked === true || currentDayIndex - (trigger1LogArr[trigger1LogArr.length - 1].trig1AtIndex) > 32)
        return [false, trigger12LogArr]
    ...
    function trigger12Log(currentDayList, log1) {
        let currentDayIndex = currentDayList.length - 1
        let currentDayData = currentDayList[currentDayIndex]

        let log12ProfileN = {}
        log12ProfileN.name = dataName
    ...
    if (cond2 === true) {
        let log1 = trigger1LogArr[trigger1LogArr.length - 1]
        log1.trig1Fucked = true
        trigger12Log(currentDayList, log1)
    }

    return [cond2, trigger12LogArr]
}
...
function restoreLog() {
    ...
    let lastDay = 大宗策略cod1.length > 0 ? 大宗策略cod1[大宗策略cod1.length - 1].trig1Date : ""
    let getLastLogDateIndexFunc = function (dataName, dayDatas) {
        let lastDayIndexIn = 70
        let trigger1LogArr = 大宗策略cod1.filter((item) => { return item.name == dataName })
        let trigger12LogArr = 大宗策略cod12.filter((item) => { return item.name == dataName })
        if (trigger1LogArr.length > 0 && lastDay) {
            lastDayIndexIn = dayDatas.findIndex(ele => { return ele.date == lastDay })
        }
        console.log(dataName, lastDay, lastDayIndexIn)
        return [trigger1LogArr, trigger12LogArr, lastDayIndexIn]
    }
...
async function down1Back1(nameCodes, backName) {
    ...
    for (let i = 0; i < nameCodes.length; i++) {
        ...
        [trigger1LogArr, trigger12LogArr, lastLogIndex] = getLastLogDateIndexFunc(dataName, dayDatas);
        ...
        for (let currentDayIndex = lastLogIndex + 1; currentDayIndex <= dayDatas.length - 1; currentDayIndex++) {
            ...
            //https://juejin.cn/post/7311681326712995903 加分号;
            [conditon1, trigger1LogArr] = backTest1(dataName, dayDatas, currentDayIndex, currentDayList, currentWeekList, currentMonthList, trigger1LogArr);
            [conditon2, trigger12LogArr] = backTest12(dataName, dayDatas, currentDayIndex, currentDayList, trigger1LogArr, trigger12LogArr);
        }

        nameCodes[i].trigger1LogArr = trigger1LogArr
        nameCodes[i].trigger12LogArr = trigger12LogArr

        log1All = log1All.concat(trigger1LogArr)
        log12All = log12All.concat(trigger12LogArr)
        trigger1LogArr.forEach(item => { console.log(item.trig1Date, item.trig1AtIndex, item.trig1Fucked) })
        trigger12LogArr.forEach(item => { console.log(item.fuckTrig1, item.trig12Date) })
    }

    let log1AllStr = `var ${backName}cod1 = ` + JSON.stringify(log1All, null, 0) + "\r\n"
    let log12AllStr = `var ${backName}cod12 = ` + JSON.stringify(log12All, null, 0) + "\r\n"
    let promise = new Promise((resolve, reject) => {
        fs.writeFile(`${folder}${backName}.js`, log1AllStr + log12AllStr, 'utf8', (err) => {
