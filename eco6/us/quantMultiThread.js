...
const { Worker, isMainThread, setEnvironmentData, getEnvironmentData, parentPort, workerData } = require('node:worker_threads');
const { dayToPeriod, xueqiuFormatDate } = require("../cn/jslib/my.js");
...
function writeDataToFile(dataName, dayDatas) {
...
function getDataFromFile(dataName, justCheck = false) {
    let promise = new Promise((resolve, reject) => {
...
async function getDataFromUrl(dataName, dataCode) {
    ...
    if (!browser) {
        ...
        browser.on('disconnected', () => { browser = null; indexPage = null; })
    }
...
function backTest(dataName, dayDatas) {
    ...
    function mylog(dataName, dayDatas, currentDayList, currentWeekList, currentMonthList, cond1) {
        ...
        if (!isMainThread) {//cmdLog
            parentPort.postMessage({ name: workerData.dataName, trigDate: currentDayData.date, cmdlogNbspCount: cmdlogNbspCount })
        }
    }
    ...
    if (dataName == "标普500_xueqiu_day") { logNameDateCondAtter = typeof 标普500策略 === 'undefined' ? [] : 标普500策略; cmdlogNbspCount = 0; }
    if (dataName == "道琼斯_xueqiu_day") { logNameDateCondAtter = typeof 道琼斯策略 === 'undefined' ? [] : 道琼斯策略; cmdlogNbspCount = 1; }
    if (dataName == "纳指_xueqiu_day") { logNameDateCondAtter = typeof 纳指策略 === 'undefined' ? [] : 纳指策略; cmdlogNbspCount = 2; }
    if (logNameDateCondAtter.length > 0) {
        let lastDay = logNameDateCondAtter[logNameDateCondAtter.length - 1].trigDate
    ...
    for (var currentDayIndex = startDayIndex; currentDayIndex <= dayDatas.length; currentDayIndex++) {
        ...
        if (lastResult) mylog(dataName, dayDatas, currentDayList, currentWeekList, currentMonthList, currentDay, volumeUp)
        restAction()
    }
    //等效setInterval循环

    return logNameDateCondAtter
}
...
async function main() {
    ...
    if (isMainThread) {
        ...
        for (let i = 0; i < nameCodes.length; i++) {
            ...
            let dayDatas = await getDataFromFile(dataName)
            if (!dayDatas) {
                dayDatas = await getDataFromUrl(dataName, dataCode)
                await writeDataToFile(dataName, dayDatas)
            }
            ...
            nameCodes[i].nameConsole = false
            nameCodes[i].work = new Worker(__filename, { workerData: { dataName: dataName, dayDatas: dayDatas } })
            nameCodes[i].work.on('message', (workerResData) => {
                if (workerResData.trigDate) {//cmdLog
                    let repetNbsp = "                     ".repeat(workerResData.cmdlogNbspCount)
                    if (!nameCodes[i].nameConsole) { console.log(repetNbsp, workerResData.name); nameCodes[i].nameConsole = true; }
                    console.log(repetNbsp, workerResData.trigDate)
                    return
                }
                threadCount += 1
                nameCodes[i].logNameDateCondAtter = workerResData.logNameDateCondAtter
                logAllDates += `var ${workerResData.name.split("_")[0]}策略 = ` + JSON.stringify(workerResData.logNameDateCondAtter, null, 0) + "\r\n"
                if (threadCount === nameCodes.length) fs.writeFileSync(`${folder}美股指数策略.js`, logAllDates, { encoding: 'utf8' })
            })
        }

        let beautifyConsoleData = []
        // for (let i = 0; i < nameCodes.length; i++) {
        //     let tmpData = nameCodes[i].logNameDateCondAtter.map((item) => {
        //         return [item.name, item.trigDate]
        //     })
        //     beautifyConsoleData = beautifyConsoleData.concat(tmpData)
        // }
        ...
        标普500策略 = 标普500策略.map((item) => { return [item.name, item.trigDate] })
        道琼斯策略 = 道琼斯策略.map((item) => { return [item.name, item.trigDate] })
        纳指策略 = 纳指策略.map((item) => { return [item.name, item.trigDate] })
        beautifyConsoleData = [...标普500策略, ...道琼斯策略, ...纳指策略].sort((a, b) => Date.parse(a[1]) - Date.parse(b[1]))
        console.log(beautifyConsoleData)

    }
    else {
        let logNameDateCondAtter = backTest(workerData.dataName, workerData.dayDatas)
        parentPort.postMessage({ name: workerData.dataName, logNameDateCondAtter: logNameDateCondAtter })
    }
}
main()
